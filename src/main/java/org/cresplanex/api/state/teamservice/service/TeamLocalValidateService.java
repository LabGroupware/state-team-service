...
import org.cresplanex.api.state.common.saga.local.team.*;
...
public class TeamLocalValidateService extends BaseService {
    ...
    public void validateCreatedTeam(String organizationId, String name, String description, String ownerId, List<String> userIds)
        ...
        if (userIds.size() != userIds.stream().distinct().count()) {
            throw new WillAddedTeamUserDuplicatedException(userIds);
        }

        if (userIds.contains(ownerId)) {
            throw new NotAllowedTeamUsersContainOwnerException(userIds, ownerId);
        }
    ...
    public TeamEntity validateTeam(String teamId, List<String> userIds, String actionType)
